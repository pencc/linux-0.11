...
// 其中最后的__res是其输出值。
// **流程：保存fs寄存器的值、用seg设置fs、取seg:addr处1字节内容到al寄存器、恢复fs寄存器原内容
// “=a”中的“a”称为加载代码，“=”表示这是输出寄存器，含义是在这段代码运行结束后将eax所代表的的寄存器的值放入_res变量中
// “0”，在这段代码开始运行时将seg放到eax寄存器中，“0”表示使用与上面相同位置上的输出寄存器
// “m”代表使用内存地址
