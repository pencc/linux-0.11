...
// 就需要传递指针'*p'的指针，即'**p'.
/* TASK_UNINTERRUPTIBLE状态存在的意义就在于，内核的某些处理流程是不能被打断的。如果响应异步信号，
 * 程序的执行流程中就会被插入一段用于处理异步信号的流程（这个插入的流程可能只存在于内核态，也可能延
 * 伸到用户态），于是原有的流程就被中断了。（kill -9杀不死一个处于TASK_UNINTERRUPTIBLE状态的进程）
 */
